...
int main(int argc, const char* argv[]) {
  ...
  const Config config(socket_type, 600 * 1024, 0.0);
  socket->Send(config.AsString());

  uint16_t port = atoi(socket->Receive(16).c_str());
  ...
  std::string recv = mbm_socket->Receive(chunk_len);
  while (recv.find(END_OF_LINE) == std::string::npos) {
    std::cout << "." << std::flush;
//    if (recv.empty())
//      break;
    recv = mbm_socket->Receive(chunk_len);
  }
  std::cout << recv.substr(recv.find(END_OF_LINE) + strlen(END_OF_LINE) - 1,
                           recv.length()) << "\n";
  std::cout << mbm_socket->Receive(chunk_len) << "\n";
  return 0;
}
